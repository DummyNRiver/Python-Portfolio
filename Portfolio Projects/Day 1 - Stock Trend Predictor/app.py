...
# Load the model 
model = load_model(r'C:\Users\jaraneses\OneDrive - 2X LLC\Codes\Stock Trend Predictor\stock_dl.h5')
...
def index():
    if request.method == 'POST':
        ...
        if not stock:
            stock = 'GOOGL' # The default stock, if none is entered
        ...
        if df is None or df.empty:
            err = f"No data found for '{stock}'. Please check and enter the correct stock code (e.g., AAPL for Apple)."
        ...
        ax2.plot(ema200, 'g', label = 'EMA 200')
